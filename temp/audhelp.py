...
class WaveSink(object):
    ...
    def restart(self, fp='out.wav'):
        self.close()
        self._fp = open(fp, 'wb')
        self._wavep = wave.open(self._fp, 'wb')
        self._wavep.setsampwidth(self._sample_width)
        self._wavep.setnchannels(1)
        self._wavep.setframerate(self._sample_rate)
...
class ConversationStream(object):
    ...
    Args:
      ...
      sample_width: size of a single sample in bytes.
    """
    def __init__(self, source, sink, iter_size, sample_width, audio_type, audio_out):
        self._source = source
        self._sink = sink
    ...
    def restart_wav(self):
        if self._audio_type == 'wav':
            self._source.restart()

    def restart_wav_out(self):
        if self._audio_out == 'wav':
            self._sink.restart()
